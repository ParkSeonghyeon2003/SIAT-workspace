...
from audio import whisper_process
...
with st.sidebar:
    ...
    st.write("### 설정")
    st.write("대화 내용과 conversations_id는 JSON 파일에 저장됩니다.")

    uploaded_file = st.file_uploader("오디오 파일 업로드하세요 (mp3, wav)", type=["mp3", "wav"], key=f"audio_uploader_{st.session_state.uploader_key}")
...
if prompt := st.chat_input("메시지를 입력하세요"):
    ...
    if prompt in whisper_commands:
        if not uploaded_file:
            st.warning("오디오 파일을 업로드해주세요.")
            st.stop()

        mode = whisper_commands[prompt]

        with st.spinner(f"{prompt} 중입니다... 잠시만 기다려 주세요."):
            assistant_text = whisper_process(client, uploaded_file, mode)
    else:
        # AI 응답 받기
        try:
